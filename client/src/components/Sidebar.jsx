// ================= Sidebar Component =================
// Handles interview settings: job role, difficulty, interview type, real-time score, and AI tips
import React from "react";
import { Briefcase, Target, Lightbulb, TrendingUp, Check, ChevronDown } from "lucide-react";
                ...
                {jobRoles.map((role) => (
                  <Select.Item
                    ...
                    className="relative flex items-center px-4 py-2 cursor-pointer rounded-lg text-white select-none
                               hover:bg-[#3A7BFF]/10 outline-none border-none ring-0
          ...
          {difficulties.map((lvl) => (
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-lg text-xs transition-all 
                ${difficulty === lvl ? "bg-[#3A7BFF] text-white" : "bg-white/5 text-gray-400 hover:bg-white/10"}`}
            >
              {lvl}
            </button>
          ))}
        ...
        <div className="flex gap-2">
          {interviewTypes.map((type) => (
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-lg text-xs transition-all 
                ${interviewType === type ? "bg-[#3A7BFF] text-white" : "bg-white/5 text-gray-400 hover:bg-white/10"}`}
            >
              {type}
            </button>
          ))}
